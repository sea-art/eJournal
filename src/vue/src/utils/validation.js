...
export default {
    ...
    validateURL (url, displayMessage = true, checkProtocol = true) {
        ...
        if (checkProtocol) {
            let protocolRe = /^(http|https|ftp|ftps):\/\/.*/
            var validProtocol = protocolRe.test(url)

            if (displayMessage && !validProtocol) { Vue.toasted.error('The given URL is not valid, did you include the protocol e.g. http://?') }
        }
