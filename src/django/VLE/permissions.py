...
ASSIGNMENT_PERMISSIONS = ['can_edit_assignment', 'can_view_all_assignment_journals', 'can_grade',
...
def has_assignment_permission(user, permission, assignment):
    ...
    # Apply negations
    if permissions['can_have_journal'] and permissions['can_view_all_assignment_journals']:
...
def is_user_supervisor_of(supervisor, user):
    ...
    has the permission can_view_all_assignment_journals."""
    for course in supervisor.participations.all():
        if supervisor.has_permission('can_view_course_users', course):
            ...
            for assignment in course.assignment_set.filter(journal__user=user):
                if supervisor.has_permission('can_view_all_assignment_journals', assignment):
                    return True
