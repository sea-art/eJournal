...
class User(AbstractUser):
    ...
    def can_view(self, obj):
        if isinstance(obj, Journal):
            if obj.user != self:
                return self.has_permission('can_view_all_assignment_journals', obj.assignment)
            else:
...
class Role(models.Model):
    ...
    can_view_all_assignment_journals = models.BooleanField(default=False)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.can_view_all_assignment_journals and self.can_have_journal:
            raise ValidationError('An administrative user is not allowed to have a journal in the same course.')

        if self.can_grade and not self.can_view_all_assignment_journals:
            raise ValidationError('A user needs to be able to view journals in order to grade them.')

        if self.can_publish_grades and not (self.can_view_all_assignment_journals and self.can_grade):
            raise ValidationError('A user should not be able to publish grades without being able to view or grade \
                                  the journals.')

        if self.can_comment and not (self.can_view_all_assignment_journals or self.can_have_journal):
